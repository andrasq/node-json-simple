...
JsonScanner.prototype = {
    ...
    function scanString( str, i0, spanOnly ) {
        ...
        while (++i < len) {
            ...
            else if (ch === '"') return [type, i0, i+1 /*, str.slice(i0, i+1)*/];
        }
...
function parse_r( str, term ) {
    switch (term[0]) {
    case 's':
        return JSON.parse(str.slice(term[1], term[2]));
        break;
    case 't':
        return str.slice(term[1]+1, term[2]-1);
        break;
    case 'l':
        return JSON.parse(str.slice(term[1], term[2]));
        break;
    case 'a':
        ...
        for (i=0; i<a.length; i++) ret.push(parse_r(str, a[i]));
        return ret;
        break;
    case 'o':
        var i, a = term[3], ret = {};
        for (i=0; i<a.length; i++) ret[parse_r(str, a[i][0])] = parse_r(str, a[i][1]);
        return ret;
        break;
    default:
...
assert.deepEqual(parse_r(json, v), JSON.parse(json));
...
var scan = module.exports;
t1 = Date.now();
for (i=0; i<10000; i++) parse_r(json, scan(json));
