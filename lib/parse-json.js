...
function containsEscapes( s, i, j ) {
    for ( ; i<j; i++) if (s.charCodeAt(i) === 0x5c) return true;
    return false;
};

function decodeString( s, i, j ) {
    //if (jsonlib.hasEscape(s, i+1, j-1)) return JSON.parse(s.slice(i, j));
    if (containsEscapes(s, i+1, j-1)) return JSON.parse(s.slice(i, j));
    else return s.slice(i+1, j-1);
}
...
function scanArray( str, i, retval ) {
    ...
    var i0 = i, e1, ret = {v: 0}, len = str.length;
    i++;
    ...
    while (str[i] !== ']' && i < len) {
        ...
        e1 = scanTerm(str, i, ret);
        // have (i)value(e1)
        if (retval) values.push(ret.v);
        i = e1;
        i = skipWhitespace(str, i);
        if (str[i] === ',' && str[i+1] !== ']') i++;
        else if (str[i] === ']') break;
...
function scanObject( str, i, retval ) {
    ...
    values.isHash = true;
    var hash = {};
    var i0 = str, ch, e1, e2, ret2 = {v: null}, len = str.length;
    i++;
    ...
    while (str[i] !== '}' && i < len) {
        ...
        if (ch === '"') {
            ...
            e2 = scanTerm(str, i, ret2);
            // have (iw)name(e1):value(e2)
            if (retval) {
                var name = decodeNames ? decodeString(str, iw, e1) : str.slice(iw, e1);
                if (decodeObjects) hash[name] = ret2.v;
                else values.push([name, ret2.v]);
            }
            i = skipWhitespace(str, e2);
            if (str[i] === ',') i++;
            else if (str[i] === '}') break;
            else throw new Error(makeScanError("unrecognized char in object", str, i));
        }
        else if (ch === '}') break;
        else throw new Error(makeScanError("unexpected character in object", str, i));
    }
    if (retval) {
        if (decodeObjects) retval.v = hash;
        else retval.v = values;
        //if (retval) retval.v = str.slice(i0, i+1);
    }
    return i+1;
}
...
t1 = Date.now();
for (i=0; i<10000; i++) scanTerm(json, 0);
